...
def testeqsupport():
  ...
  if rc != 0:
    ...
    msg += '  latex: ' + str(p.stdout.readlines()[0].rstrip()) + '.\n'
  ...
  if rc != 0:
    ...
    msg += '  dvipng: ' + str(p.stdout.readlines()[0].rstrip()) + '.\n'
...
def standardconf():
  ...
  <link rel="stylesheet" href="jemdocmobile.css" type="text/css" />
...
def getNameorClass(a):    
    b = a.split(' ')
    out = ''
    for str in b:        
        pattern = re.compile(r'\+([A-Za-z]+):')   
        resul = pattern.findall(str)
        if len(resul) > 0:
            info = str.split(':')
            info = info[1]
            out += resul[0] + '=' + '\\"' + info + '\\" '    
            a = a.replace('+' + resul[0] + ':' + info, '')    
    return (out, a.strip())
...
def replacelinks(b):
  ...
  while m:
    ...
    (a_name, lk) = getNameorClass(link)
    if len(a_name) > 0:       
      b = b[:m.start()] + r'<a %s>%s<\/a>' % (a_name.strip(), linkname) + b[m.end():]      
    else:
      (a_name, linkname) = getNameorClass(linkname)
      # if '+name:' in linkname:
      #   a_name = linkname.split(' ')
      ...
      #   print(a_name)
      #   a_name = a_name.strip()
      #   linkname = linkname.replace('+name:' + a_name, '')
      #   a_name = 'name=\\"'+ a_name + '\\"'
      #   print(a_name)
      #   print(linkname)
      # else:
      #   a_name = ''
      b = b[:m.start()] + r'<a %s href=\"%s\"%s>%s<\/a>' % (a_name, link, option, linkname) + b[m.end():]
  ...
  while 1: # wait for EOF.
    ...
    # look for titles.
      ...
      if '+top' in na:
        na = na.replace('+top', '')
        na = na.strip()
        mystr = '<h%d>|<a class="top" href="#tlayout"> [back to top]</a></h%d>\n'
      else:
        mystr = '<h%d>|</h%d>\n'       
      hb(f.outf, mystr % (c, c), br(na, f))
      #hb(f.outf, mystr % (c, c), br(news[0], f))
...
def main():
  ...
  for inname in innames:
    ...
    outfile = io.open(thisout, 'w', encoding = 'utf8')
